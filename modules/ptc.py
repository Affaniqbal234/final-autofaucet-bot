...
async def process_ptc_list(page, ptc_url):
    try:
        ...
        print_success(f"Moved to SurfAds: {ptc_url}")
        ...
        while True:
            with suppress(Exception):
                ...
                if "All Available Ads Watched" in text1:
                    print_info("[SurfAds] All Available Ads Watched.")
                    print_success("[SurfAds] Done")
                    return
            
            print_wait("[SurfAds] Waiting for Submit button to appear in 45 seconds")
